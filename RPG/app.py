요청해주신 밸런스 조정과 기능 개선 사항을 반영했습니다!

### 🔄 수정 및 변경 사항

1. **난이도 & 성장에 따른 밸런스 조정**:
* **레벨업 속도 증가**: 필요 경험치(`max_exp`) 상승 폭을 낮추어 레벨이 훨씬 빠르게 올라갑니다.
* **몬스터 대폭 강화**: 슬라임, 고블린, 드래곤의 HP와 공격력을 상향 조정했습니다.


2. **명칭 변경**:
* "코딩 블록 작성" ➡️ "전투 설정 (자동 전투 알고리즘)"으로 수정했습니다.


3. **음식 아이템 시스템 개선**:
* 상점에서 음식 구매 시 즉시 회복되지 않고 **인벤토리(가방)로 들어갑니다.**
* 전투 설정 조건에 **"내 HP가 일정 수준 이하일 때 음식 섭취"** 옵션을 추가하여 전투 중 자동 회복 기능을 만들었습니다.


4. **사이드바 UI 개선**:
* **인벤토리 & 보유 스킬 목록** 표시 추가.
* 능력치 설명에 마우스를 가져다 대면 툴팁이 뜨도록 `help` 옵션(상세정보 툴팁)을 적용했습니다.
